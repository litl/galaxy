...
func unregisterAll(c *cli.Context, signals chan os.Signal) {
	...
	<-signals
	unregisterShuttle(c)
	serviceRuntime.UnRegisterAll(utils.GalaxyEnv(c), utils.GalaxyPool(c), c.GlobalString("hostIp"))
	os.Exit(0)
}
...
func registerAll(c *cli.Context, loggedOnce bool) {
	...
	registrations, err := serviceRuntime.RegisterAll(utils.GalaxyEnv(c), utils.GalaxyPool(c), c.GlobalString("hostIp"))
	if err != nil {
...
func register(c *cli.Context) {
	...
	err := serviceRuntime.RegisterEvents(utils.GalaxyEnv(c), utils.GalaxyPool(c),
		c.GlobalString("hostIp"), containerEvents)
	if err != nil {
	...
	for {
		...
		case ce := <-containerEvents:
			switch ce.Status {
			case "start":
				reg, err := serviceRegistry.RegisterService(utils.GalaxyEnv(c), utils.GalaxyPool(c),
					c.GlobalString("hostIp"), ce.Container)
				if err != nil {
				...
				registerShuttle(c)
			case "die", "stop":
				reg, err := serviceRegistry.UnRegisterService(utils.GalaxyEnv(c), utils.GalaxyPool(c),
					c.GlobalString("hostIp"), ce.Container)
				if err != nil {
